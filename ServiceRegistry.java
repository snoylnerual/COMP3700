...
public class ServiceRegistry {
    ...
    private static void serve(Socket socket, int clientID) throws IOException {
        ...
        if (req.code == ServiceMessageModel.SERVICE_PUBLISH_REQUEST) {  //this adds a service to the service list
        ...
        if (req.code == ServiceMessageModel.SERVICE_DISCOVER_REQUEST) { // looking for certain service using service code
            ...
            if (list == null) {
                res.code = ServiceMessageModel.SERVICE_DISCOVER_NOT_FOUND;  // no services found
                res.data = "";
            }
            else {
                res.code = ServiceMessageModel.SERVICE_DISCOVER_OK; // returns a random service from this list of matched code
